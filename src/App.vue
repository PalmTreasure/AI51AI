...
import { collection, doc, getDoc, updateDoc, onSnapshot, increment } from "firebase/firestore";
...
export default {
  ...
  data() {
    return {
      ...
      // قائمة المكافآت
      rewardsList: [
        { members: 10, salary: 50 },
        { members: 20, salary: 100 },
        { members: 30, salary: 150 },
        { members: 40, salary: 200 },
        { members: 50, salary: 250 },
        { members: 60, salary: 300 },
        { members: 70, salary: 350 },
        { members: 80, salary: 400 },
        { members: 90, salary: 450 },
        { members: 100, salary: 500 },
        { members: 120, salary: 550 },
        { members: 140, salary: 600 },
        { members: 160, salary: 650 },
        { members: 180, salary: 700 },
        { members: 200, salary: 750 },
        { members: 220, salary: 800 },
        { members: 240, salary: 850 },
        { members: 260, salary: 900 },
        { members: 280, salary: 950 },
        { members: 300, salary: 1000 }
      ],
      ...
      translations: {
        ...
        EN: {
          ...
          vip2Required: 'Each member joining your team must purchase VIP 2 level to be counted in the rewards system.',
  ...
  created() {
    ...
    onAuthStateChanged(auth, (user) => {
      ...
      if (user) {
        ...
        // إلغاء المراقبة والمؤقت إذا كان المستخدم غير مسجل
  ...
  methods: {
    ...
    monitorBlockStatus(userId) {
      ...
      this.blockUnsubscribe = onSnapshot(userRef, async (docSnap) => {
        if (docSnap.exists()) {
          ...
          if (userData.blocked === true) {
            ...
            // تأكد أن المستخدم لا يزال مسجل الدخول
            if (currentUser && currentUser.uid === userId) {
              // عرض نافذة الحظر بدلاً من alert
        ...
        console.error("خطأ في مراقبة حالة الحظر:", error);
      });
    },

    // ==================== التعامل مع نافذة الحظر ====================
    ...
    async settleAndReward(userId) {
      try {
        ...
        const earnedProfit = profitPerSecond * secondsPassed;
        
        if (earnedProfit > 0) {
          await updateDoc(userRef, {
            balance: increment(earnedProfit)
          });
        ...
        console.error("خطأ في تسوية أرباح VIP:", error);
      }
    },
    
    // تسجيل أرباح VIP في السجلات
    async logVipProfit(userId, amount, vipLevel) {
      try {
        const historyRef = collection(db, "users", userId, "vip_profits");
        ...
        console.error("خطأ في تسجيل أرباح VIP:", error);
      }
    },
...
.lang-menu {
  ...
  background: linear-gradient(135deg, #11151C, #1A1F2A);
  border-radius: 20px;
  box-shadow: 0 10px 40px rgba(0, 0, 0, 0.5), 0 0 0 2px #D4AF37;
  z-index: 9998;
  overflow: hidden;
  min-width: 250px;
...
.modal-container {
  ...
  max-height: 90vh;
  overflow: hidden;
  box-shadow: 0 20px 60px rgba(0, 0, 0, 0.6), 0 0 0 2px #D4AF37;
...
.partnerships {
  background: linear-gradient(135deg, rgba(212, 175, 55, 0.1), rgba(246, 226, 122, 0.05));
  border: 1px solid #D4AF37;
  border-radius: 12px;
  padding: 15px 20px;
  display: flex;
