...
export default {
  ...
  methods: {
    ...
    saveAvatarToLocal(avatarData) {
      if (avatarData) {
        localStorage.setItem(`user_avatar_${this.userData.uid}`, avatarData);
      } else {
        localStorage.removeItem(`user_avatar_${this.userData.uid}`);
      }
    },

    loadLocalAvatar() {
      const savedAvatar = localStorage.getItem(`user_avatar_${this.userData.uid}`);
    ...
    async saveProfileChanges() {
      ...
      try {
        ...
        const updateData = { username: this.editUsername.trim() };
        ...
        await updateDoc(userRef, { username: updateData.username });
        
        if (updateData.username !== this.userData.username) {
          try {
            await updateProfile(user, { displayName: updateData.username });
          } catch (e) {
